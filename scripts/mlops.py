...
def step_two(path, subs, parameters):
    ...
    ensemble_data = pd.concat(
        [pd.read_csv(path / subs[i], index_col="id") for i in range(parameters["number_files"])], axis=1
    )
...
def step_four(X_train, X_val, y_train, ensemble_data, parameters):
    ...
    # Создание и обучение модели -- поиск по сетке
    model = XGBClassifier()
    parameters.pop("number_files")
    clf = GridSearchCV(model, parameters, cv=2, verbose=False)

    clf.fit(X_train, y_train)
    ensemble_model = clf.best_estimator_
...
# Просто параметры -> поиск по сетке + раскладываем на GPU
parameters_grid = {
    "max_depth": [5, 10],
    ...
    "random_state": [42],
    "number_files": 5000,
    "tree_method": ["gpu_hist"],  # выключить, если нет GPU
}
...
def executing_pipeline(parameters):
    ...
    task = Task.create(
        project_name="TPS_project/.pipelines/TPS_name_project",
        task_name="TPS_name_project #38",
    )
...
if __name__ == "__main__":
    ...
    executing_pipeline(parameters_grid)
